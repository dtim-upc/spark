...
import org.apache.spark.sql.catalyst.expressions.{Attribute, AttributeReference, Cast, Divide, Expression, GenericInternalRow, IsNull, Length, Log2, Multiply}
...
import org.apache.spark.sql.functions.{col, desc, expr, lit, log2, lower, size, soundex, split, sum, trim, typedLit, udf, when}
...
//              }
              if (selectMetaFeaturesCustom.size > 0) {
                df = computeCustom(df, ds, nominalA(tuple._2), selectMetaFeaturesCustom)
              }
...
//      case _ => ds.select(aggExprs: _*).queryExecution.toRdd.collect().head
  ...
  def computeCustom(metaDF: DataFrame, ds: Dataset[_], colName: Attribute, mf: Seq[MetaFeature])
  : DataFrame = {
    ...
    mf.map(meta => meta.key match {
      ...
      case TypeInformation.key =>
        ...
        val tmp = ds.select(colName.name).na.drop.distinct
